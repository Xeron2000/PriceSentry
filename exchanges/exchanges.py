...
class Exchange:
    ...
    async def _ws_connect(self, symbols):
        ...
        logging.info(f"Attempting to establish WebSocket connection for {self.exchangeName}, subscribing symbols: {symbols}")
            ...
            try:
                if self.exchangeName == "okx":
                    ...
                    async with websockets.connect(uri) as websocket:
                        ...
                        logging.info(f"OKX WebSocket connection established")
                        
                        # Send subscription request
                        await websocket.send(json.dumps(subscribe_msg))
                        logging.info(f"Subscription request sent to OKX")
                            ...
                            try:
                                ...
                                if "data" in data:
                                    for item in data["data"]:
                                        ...
                                        self.historical_prices[symbol] = [
                                            item for item in self.historical_prices[symbol] 
                                            if item[0] >= cutoff
                                        ]
    ...
    def start_websocket(self, symbols):
        ...
        logging.info(f"Starting WebSocket connection for {self.exchangeName}, number of symbols: {len(symbols)}")
        ...
        def run_websocket_loop():
            logging.info(f"WebSocket thread started, creating new event loop for {self.exchangeName}")
            ...
            try:
                ...
                logging.info(f"WebSocket thread ending, closing event loop")
                loop.close()
        ...
        if not self.ws_connected:
            logging.error(f"WebSocket connection establishment failed, timeout")
            raise ConnectionError("Failed to establish WebSocket connection")
        
        logging.info(f"WebSocket connection successfully established, exchange: {self.exchangeName}")
        ...
        if not self.ws_connected:
            ...
            try:
                for symbol in symbols:
                    ...
                    ohlcv = self.exchange.fetch_ohlcv(symbol, '1m', since=since, limit=1)
    ...
    def check_ws_connection(self):
        ...
        if not self.ws_connected and self.running:
            logging.warning(f"{self.exchangeName} WebSocket connection disconnected, attempting to reconnect")
