...
def generate_config():
    ...
    notification_choice = get_input("输入选择 (1/2/3)", "1")
    if notification_choice in ["1", "3"]:
        notification_channels.append("telegram")
    if notification_choice in ["2", "3"]:
        notification_channels.append("dingding")
    ...
    if "dingding" in notification_channels:
        print("\n🔔 钉钉配置")
        dingding_webhook = get_input("钉钉 Webhook URL", required=False)
        dingding_secret = get_input("钉钉签名密钥", required=False)
