...
class TestLoadConfig:
    ...
    def test_load_config_missing_required_key(self):
        ...
        config_data = {
            'exchange': 'binance',
            'symbolsFilePath': 'config/symbols.txt',
            # Missing defaultTimeframe, defaultThreshold, notificationChannels, notificationTimezone
        }
        
        with patch('builtins.open', mock_open(read_data=yaml.dump(config_data))), \
             patch('utils.load_config.logging') as mock_logging:
            
            with pytest.raises(ValueError, match="Missing required config key: defaultTimeframe"):
