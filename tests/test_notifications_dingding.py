...
class TestDingDingNotification:
    ...
    def test_send_dingding_message_network_error(self):
        ...
        with patch('notifications.dingding.requests.post') as mock_post:
            with patch('builtins.print') as mock_print:
                ...
                assert result is False
                mock_print.assert_called_with("Error while sending DingDing message: Network error")

    def test_send_dingding_message_with_secret(self):
        """Test DingDing message sending with secret."""
        with patch('notifications.dingding.requests.post') as mock_post, \
             patch('notifications.dingding.time.time', return_value=1640995200), \
             patch('notifications.dingding.hmac.new') as mock_hmac, \
             patch('notifications.dingding.base64.b64encode') as mock_b64encode, \
             patch('notifications.dingding.urllib.parse.quote_plus') as mock_quote_plus:
    ...
    def test_send_dingding_message_signature_generation(self):
        ...
        with patch('notifications.dingding.requests.post') as mock_post, \
            ...
            def mock_hmac_new(key, msg, digestmod):
                mock_hmac = Mock()
                mock_hmac.digest.return_value = b"test_signature_bytes"
                return mock_hmac
            
            with patch('notifications.dingding.hmac.new', side_effect=mock_hmac_new), \
                 patch('notifications.dingding.base64.b64encode', return_value=b"dGVzdF9zaWduYXR1cmU="), \
                 patch('notifications.dingding.urllib.parse.quote_plus', return_value="test_signature_encoded"):
                ...
                assert call_args[1]['digestmod'].__name__ == 'sha256'
    ...
    def test_send_dingding_message_url_encoding(self):
        ...
        with patch('notifications.dingding.requests.post') as mock_post, \
            ...
            with patch('notifications.dingding.hmac.new') as mock_hmac, \
                 patch('notifications.dingding.base64.b64encode') as mock_b64encode, \
                 patch('notifications.dingding.urllib.parse.quote_plus') as mock_quote_plus:
    ...
    def test_send_dingding_message_timestamp_precision(self):
        ...
        with patch('notifications.dingding.requests.post') as mock_post, \
             patch('notifications.dingding.time.time', return_value=1640995200.123):
